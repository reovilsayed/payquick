...
class WC_QuickPay_API_Transaction extends WC_QuickPay_API {
	...
	public function create( WC_Order $order ) {
		...
		$order_params = WC_QuickPay_Order_Payments_Utils::prepare_transaction_params( $order );
	...
	public function patch_link( $transaction_id, WC_Order $order ) {
		...
		$base_params = [
			...
			'payment_methods'              => apply_filters( 'woocommerce_quickpay_cardtypelock_' . $payment_method, WC_QP()->s( 'quickpay_cardtypelock' ), $payment_method ),
	...
	public static function is_transaction_caching_enabled(): bool {
		$is_enabled = ! ( strtolower( WC_QP()->s( 'quickpay_caching_enabled' ) ) === 'no' );
	...
	public function cache_transaction(): bool {
		...
		return set_transient( 'wcqp_transaction_' . $this->resource_data->id, json_encode( $this->resource_data, JSON_THROW_ON_ERROR ), $expiration );
	}
